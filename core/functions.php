...
function createAccessFile($obj_id, $new_name, $old_name, $page_id, $section_id, $subdir) {
        global $admin, $database, $MESSAGE;

    if (!function_exists('page_filename')){require(WB_PATH.'/framework/functions.php');}

    $sPagesDir = WB_PATH.PAGES_DIRECTORY;
        $obj_link = '/'.$subdir."/".page_filename($new_name)."-".$obj_id;
        $obj_link_old = $old_name !== null ? '/'.$subdir."/".page_filename($old_name)."-".$obj_id : null;
        
        $sObjPath = $sPagesDir.$obj_link.PAGE_EXTENSION;
        $sObjPath_old = $old_name !== null ? $sPagesDir.$obj_link_old.PAGE_EXTENSION : null;

    // Создаём директорию страниц для магазина    

    $sObjsDir = $sPagesDir.'/'.$subdir;
    if(!file_exists($sObjsDir)) {
    ...
    if(!is_writable($sObjsDir.'/')) {
        return [true, $MESSAGE['PAGES_CANNOT_CREATE_ACCESS_FILE']];
    }
    
    // удаляем старый файл, если он был
    
    if (($old_name !== null) && ($new_name != $old_name) && (is_writable($sObjPath_old))) {
        if (!unlink($sObjPath_old)) {
          return [true, $MESSAGE['PAGES_CANNOT_DELETE_ACCESS_FILE'].' - '.$sObjPath_old];
        }
    }
...
//         "\t".'define(\'POST_ID\',      '.$post_id.');'."\n".
         ...
         '// *************************************************'."\n";
      if( file_put_contents($sObjPath, $content) !== false ) {
      // Chmod the file
         change_mode($sObjPath);
      }else {
